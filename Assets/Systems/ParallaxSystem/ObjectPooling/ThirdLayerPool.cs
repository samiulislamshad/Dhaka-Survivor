...
{
    [Serializable]
    public class ThirdLayerPool : IDisposable
    {
        private readonly Dictionary<string, Stack<ThirdLayerEnvironmentObject>> _objectsByType = new();
        private readonly Dictionary<string, EnvironmentObjectData> _dataByType = new();
        private readonly ThirdLayerEnvironmentObjectFactory _factory;
        private readonly ParallaxLayerConfig _config;
        private readonly Transform _parentTransform;

        public ThirdLayerPool(
            ThirdLayerEnvironmentObjectFactory factory,
            ParallaxLayerConfig config,
            [Inject(Id = "ThirdLayerParent")] Transform parentTransform)
        {
            _factory = factory;
            _config = config;
            ...
            foreach (var envObj in _config.thirdParallaxLayer.environmentObjects)
            {
                ...
                _objectsByType[envObj.id] = new Stack<ThirdLayerEnvironmentObject>();
            }
        }
        ...
        public ThirdLayerEnvironmentObject Spawn(Vector3 position)
        {
            // Get a random ID from available objects
            if (_dataByType.Count == 0)
            {
                Debug.LogError("No objects configured in ThirdLayerPool");
                return null;
            }
        ...
        public ThirdLayerEnvironmentObject SpawnById(string id, Vector3 position)
        {
            ...
            {
                Debug.LogError($"Object with ID {id} not found in ThirdLayerPool");
                return null;
            }
        ...
        public void Despawn(ThirdLayerEnvironmentObject obj)
        {
